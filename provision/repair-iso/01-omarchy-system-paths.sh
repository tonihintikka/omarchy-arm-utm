#!/bin/bash
# Replicates on the system's fixed paths what the pacman package
# `omarchy` does (it only exists for x86_64). The quattro branch expects the tree in
# /usr/share/omarchy and the binaries on the system PATH; without that
# OMARCHY_PATH stays empty, bashrc fails and Hyprland falls into emergency mode
# for not finding /usr/share/omarchy/default/hypr/bootstrap.lua
#
...
log "1/8 tree in /usr/share/omarchy"
...
log "4/8 repo system configuration (what applies to a VM)"
